      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          <div className="hidden md:block">
            <button onClick={handleAssessmentClick} className="px-6 py-2.5 bg-slate-900 text-white text-sm font-medium rounded-full hover:bg-slate-800 transition-all hover:shadow-lg hover:-translate-y-0.5 border border-transparent">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-white border-b border-slate-100 p-6 flex flex-col gap-4 shadow-xl">
            ...
            <button onClick={handleAssessmentClick} className="w-full text-center py-3 bg-slate-900 text-white rounded-lg">Start Assessment</button>
          </div>
        )}
      </nav>
